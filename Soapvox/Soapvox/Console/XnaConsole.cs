...
namespace SandvoxConsole
{
    ...
    public class XnaConsoleComponent : DrawableGameComponent
    {
        ...
        const string Version = "Sandvox Alpha";
        ...
        private string GetStringFromKeyState(double elapsedTime)
        {
            ...
            bool ctrlPressed = CurrentKeyState.IsKeyDown(Keys.LeftControl) || CurrentKeyState.IsKeyDown(Keys.RightControl);
            foreach (KeyBinding binding in KeyboardHelper.AmericanBindings)
                if (KeyPressWithRepeat(binding.Key, elapsedTime))
                {
                    ...
                    else if (shiftPressed && !altPressed && !ctrlPressed)
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (State == ConsoleState.Closed)
            {
                if (IsKeyPressed(Keys.OemTilde)) //this opens the console
                {
                    ...
                    StateStartTime = now;
                    this.Visible = true;
                    Sandvox.BlockGame.hasFocus = false;
                }
                else
                {
                    return;
            ...
            if (State == ConsoleState.Open)
            {
                ...
                if (IsKeyPressed(Keys.OemTilde))
                {
                    ...
                    Sandvox.BlockGame.hasFocus = true;
                    return;
                }
                #endregion
                ...
                if (KeyPressWithRepeat(Keys.V, elapsedTime) && (KeyPressWithRepeat(Keys.LeftControl, elapsedTime) || KeyPressWithRepeat(Keys.RightControl, elapsedTime)))
                {
                    InputBuffer += System.Windows.Forms.Clipboard.GetText();
                    cursorPos = InputBuffer.Length;
                }
